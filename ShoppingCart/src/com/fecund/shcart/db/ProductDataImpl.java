...
public class ProductDataImpl implements ProductData_IF {
	...
	public ArrayList<Order> getAllOrderData()
	{
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("SELECT * FROM ProductOrder");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				...
				order.setUnitprice(rs.getFloat("Unitcost"));
				list.add(order);
				System.out.println(list.size());
			}
	...
	public ArrayList<Order> getNewOrderData()
	{
		...
		try {
			con = (Connection) ConnectionManager.getConnection();
			PreparedStatement ps = con.prepareStatement("SELECT * FROM ProductOrder where status='New'");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				Order order=new Order();
				order.setUserid(rs.getInt("userid"));
	...
	public ArrayList<Order> getOrderData(String username)
	{
		ArrayList<Order> list = new ArrayList<Order>();
		Connection con = null;
		try {
			con = (Connection) ConnectionManager.getConnection();
			PreparedStatement ps = con.prepareStatement("SELECT * FROM ProductOrder where username=?");
	...
	@Override
	public ArrayList<Order> updateOrderStatus(HashMap hm) {
		Iterator iter = hm.entrySet().iterator();
		while (iter.hasNext()) {
			Map.Entry mEntry = (Map.Entry) iter.next();
		String[] value= (String[]) mEntry.getValue();
			String[] key=(String[]) mEntry.getKey();
			System.out.println("in db key is="+key);
			System.out.println("in db value is="+value);
		}
	return null;
	...
	public ArrayList<Order> updateOrderStatus(String[] orderid,
		...
		try {
			...
			String query = "update ProductOrder set STATUS = ? where ID=?";
			PreparedStatement ps = con.prepareStatement(query);
	...
	public ArrayList<Order> SearchOrder(String name) {
		ArrayList<Order> list = new ArrayList<Order>();
			try {
			con = ConnectionManager.getConnection();
			ps = con.prepareStatement("select * from ProductOrder where USERNAME LIKE '%"
					+ name + "%'");
			ResultSet rs = ps.executeQuery();
			while (rs.next()) {
				Order order=new Order();
